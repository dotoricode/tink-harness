...
class TemplateTests(unittest.TestCase):
    ...
    def test_contract_rule_graph_and_verify_templates_exist(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn('HARNESS MAP', html)
            self.assertIn('Knowledge Graph', html)
            self.assertIn('Graph overview', html)
            self.assertIn('Node types', html)
            self.assertIn('uses_rule', html)
            self.assertIn('Recent run timeline', html)
            self.assertIn('MOST IMPORTANT HARNESSES', html)
            self.assertIn('CAST ROUTING RULES', html)
            self.assertIn('requirements-interview', html)
            self.assertIn('Independent verification', html)
