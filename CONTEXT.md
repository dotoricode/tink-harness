...
`/tink:forge`가 `.tink/current/`를 확정하기 전에 한 번 수행하는 내부 품질 게이트.

Grill Gate는 별도 `/tink:grill` 명령이나 실제 subagent가 아니다. 매번 내부 평가를 하지만, 품질이나 안전에 큰 영향을 주는 갈림길이 있을 때만 사용자에게 보인다. 보일 때는 proposal, reason, choices 순서로 정확히 하나의 제안만 보여준다.
...
### 소프트/하드 게이트 (Soft Gate / Hard Gate)

Grill Gate가 사용자에게 보일 때 제공하는 선택지 유형.
...
### 컨텍스트 예산 정책 (Context Budget Policy)
...
Tink는 `/tink:forge` 같은 colon namespace 표기를 쓰지 않는다. 설치 시 `.claude/commands/tink:forge.md`처럼 plugin namespace command을 만들고, 사용자는 `/tink:forge`를 실행한다.
