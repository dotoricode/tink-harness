...
Claude Code용 하네스 관리자.

Tink는 도구를 최대한 많이 쓰게 하는 시스템이 아니다. 현재 작업, 환경 제약, 사용 습관을 보고 가장 작은 유효 도구/하네스 묶음을 고르고, 없으면 좁은 하네스를 만들고, 반복 실수를 줄이며, 하네스 묶음을 계속 정리한다.

### Forge

현재 상황을 보고 가장 작은 실행 흐름을 준비하는 Tink의 대표 명령.

Forge는 단순한 하네스 생성 명령이 아니다. 작업, 환경 제약, 사용 습관을 바탕으로 하네스 선택, 하네스 만들기, 하네스 정리, 사용 습관 보정 중 필요한 판단을 하고 실행 상태를 만든 뒤 첫 안전 행동을 시작한다.
...
기존 하네스가 맞지 않을 때 조사, 실패, 사용자 피드백, 반복 작업에서 행동을 바꾸는 규칙을 추출해 새롭고 좁은 하네스를 만드는 판단.
