...
읽기 전용 helper 두 개가 이 기록을 [빠른 시작](#하네스-건강을-눈으로-확인)에서 본 로컬 대시보드로 바꿔 줍니다. 리포트는 정적 로컬 페이지입니다 — 서버, 파일 감시, hidden cache, public `tink index` 명령이 없습니다. 제안만 준비하며, 재사용 상태 변경은 기존 승인 절차를 그대로 따릅니다.
