...
- [2026-05-23] kind=mistake; source=run-20260523-git-commit-label; applies_to=global; note=AskUserQuestion 한국어 label/question을 \uXXXX 이스케이프로 수동 인코딩하면 음절 계산 오류로 엉뚱한 글자가 나온다(예: 커밋→코뱋, 코뱋 대신 커밋 필요). 한국어는 반드시 직접 문자로 작성할 것.; approval=op-20260523-korean-escape
