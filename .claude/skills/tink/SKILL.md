...
- `/tink:forge`: main path. Choose/build a harness, create run state, start work, and propose reusable learning.
...
4. If no existing harness fits, draft a small new harness instead of forcing a bad fit.
5. Ask for approval before applying, saving, purging, or honing.
6. After approval, create `.tink/current/plan.md`, `checks.md`, `steps.json`, `notes.md`, and `answers.md`.
7. Do not stop at recommendation. Execute the first safe step after run state exists.
8. Store reusable memory under `.tink/memory/` only after approval.
9. If a check fails, update `.tink/current/notes.md`, state the failure, last safe point, and next single action.
10. Keep context compact. Do not paste raw logs or full diffs.
11. Use calm, clear, concise language. No jokes.
