...
1. Harnesses만 커밋 (권장)
   ...
   실제로 커밋되는 것:
   - `.tiny/harnesses/`: 작업 방식 템플릿
   - `.tiny/config.json`: 언어/scope 같은 기본 설정
   - `.tiny/memory/`: 승인된 반복 실수, 선호, 교훈

   제외되는 것:
   - `.tiny/current/`, `.tiny/runs/`, `.tiny/cache/`: 이번 실행 기록/임시 상태
...
### 3. Hook template
Hook is optional and off by default. If the user selected the hook template in the installer, do not ask for hook scope again. Use the already selected repo/global scope.
...
무엇을 하나요?
- 일반 사용자 프롬프트를 보고 “/tink:prime을 먼저 쓰면 좋겠다”는 추천만 합니다.
- 작업을 자동 실행하지 않습니다.
- memory나 harness를 자동 저장하지 않습니다.
- `/grill-me` 같은 다른 slash skill 명령은 가로채지 않습니다. 이건 의도입니다.

지금은 hook 없이 `/tink:prime`을 직접 쓰는 흐름을 권장합니다.
```
...
다른 skill 작업에도 Tink를 적용하고 싶으면 `/tink:prime`을 먼저 실행하거나, 해당 요청에 “Tink로 먼저 하네스 추천해줘”라고 적어주세요.
```
...
- `/tink:prime`: 작업 전에 최적 harness 조합을 이유와 함께 제안하고 승인받습니다. 가장 자주 쓰는 명령입니다.
- `/tink:list`: 사용 가능한 harness를 짧게 보여줍니다.
- `/tink:save`: 승인된 새 harness 또는 개선안을 저장합니다.
